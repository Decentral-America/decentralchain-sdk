...
export function flattenCompilationResult(
...
export function getVarsDoc(
  stdlibVersion?: number,
  isTokenContext?: boolean,
  isContract?: boolean,
): IVarDoc[];

export function getFunctionsDoc(
  stdlibVersion?: number,
  isTokenContext?: boolean,
  isContract?: boolean,
): TFunction[];
...
export function repl(opts?: IReplOptions): {
  ...
  evaluate: (
    expr: string,
  ) => Promise<IDecompilationResult | IDecompilationError>;
  clear: () => void;
