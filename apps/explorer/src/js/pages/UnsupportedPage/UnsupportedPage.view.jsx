  ...
  <div className="grid-item-fixed column-6">
    <div className="line wide no-wrap">
      <div className="browser-icon">
        <img src={icon} alt={name} height={42} width={42} />
      </div>
      <div className="line bold">{name}</div>
      <div className="line btn btn-link">
        <a href={link} className="no-style" target="_blank" rel="noopener noreferrer">
          Install or update
        </a>
      </div>
    </div>
...
export const UnsupportedPageView = ({ isOpen, browsers, onClose }) => (
  <Modal className="modal-content" isOpen={isOpen} overlayClassName="modal-overlay">
    <div className="header">
      &nbsp;
      <button type="button" className="close-btn" onClick={onClose}></button>
    </div>
    <div className="row">
      <img className="icon" src={Icon} alt="Unsupported browser" height={80} width={80} />
    </div>
    <div className="row">
