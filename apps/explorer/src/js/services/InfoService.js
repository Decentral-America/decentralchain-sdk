...
const addBlockDelay = (info, formattedDelay) =>
...
export class InfoService extends ApiClientService {
  ...
  loadInfo = () => {
    ...
    return Promise.all([api.version(), this.loadHeight(), api.baseTarget()]).then(
      ([version, height, baseTarget]) => {
        return {
  ...
  loadDelay = (info) => {
    ...
    if (height < BLOCK_DELAY_INTERVAL + 1) return Promise.resolve(addBlockDelay(info, 'N/A'));

    return api.blocks.headers
      .at(height - 1)
      .then((headerResponse) => {
        return api.blocks.delay(
          headerResponse.id || headerResponse.signature,
          BLOCK_DELAY_INTERVAL,
        );
      })
      .then((delayResponse) => {
