...
const checkCryptoGen = (publicKey: string) => (bytes: Uint8Array, signature: string) =>
  verifySignature(publicKey, bytes, signature);
...
describe('Create data and signature check', () => {
  describe('Check transactions', () => {
    ...
    it('check mass transfer from node v2 signature', () =>
      checkTx(SIGN_TYPE.MASS_TRANSFER, 1).check());
    it('check mass transfer from node v3 signature', () =>
      checkTx(SIGN_TYPE.MASS_TRANSFER, 2).check());
    ...
    it('check script invocation v1 signature', () =>
      checkTx(SIGN_TYPE.SCRIPT_INVOCATION, 1).check());
    it('check script invocation v2 signature', () =>
      checkTx(SIGN_TYPE.SCRIPT_INVOCATION, 2).check());
