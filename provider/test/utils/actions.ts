...
export const Network = {
  switchTo: async function (
    ...
    if (network === 'Custom') {
      ...
      if (nodeUrl) {
        customNetworkSettings.findElement(By.css('input#node_address')).sendKeys(nodeUrl);
      }

      await customNetworkSettings.findElement(By.css('button#networkSettingsSave')).click();
    }
    ...
    await this.driver.wait(
      until.elementLocated(
        By.xpath(`//span[contains(@class, '-network-networkBottom')][text()='${network}']`),
      ),
      this.wait,
    );
  },
