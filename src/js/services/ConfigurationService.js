...
export class ConfigurationService {
  constructor(storageService) {
    ...
    this.customNetwork = global.__CONFIG__?.API_NODE_URL
      ? {
          networkId: 'custom',
          displayName: 'Custom',
  ...
  get = (networkId) => {
    if (!networkId) return this.default();

    const configuration = this.networks.find((network) => network.networkId === networkId);
    if (!configuration) throw new Error(`Configuration hasn't been found by id '${networkId}'`);
  ...
  update = ({ apiBaseUrl }) => {
    const trimmed = Strings.trimEnd(apiBaseUrl, '/');
    if (!/^https:\/\//i.test(trimmed)) {
      throw new Error('Only HTTPS URLs are allowed for API nodes');
    }
    ...
    this.customNetwork.apiBaseUrl = trimmed;
    this.customNetwork.nodes = [{ url: trimmed }];
    this.save();
  };
