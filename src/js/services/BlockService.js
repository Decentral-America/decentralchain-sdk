...
const transformBlock = (block) => {
  if (block.totalFee) {
    block.totalFee = Money.fromCoins(block.totalFee, Currency.DCC);
  }

  if (block.reward) {
    block.reward = Money.fromCoins(block.reward, Currency.DCC);
  }

  if (block.desiredReward) {
    block.desiredReward = Money.fromCoins(block.desiredReward, Currency.DCC);
  }
...
export class BlockService extends ApiClientService {
  ...
  loadSequence = (from, to) => {
    return this.getApi()
      .blocks.headers.sequence(from, to)
      .then((blocksResponse) => {
