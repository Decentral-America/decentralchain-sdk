...
const fromTokensToCoins = (valueInTokens, currencyPrecision) =>
  valueInTokens.mul(10 ** currencyPrecision).trunc();

const _fromCoinsToTokens = (valueInCoins, currencyPrecision) =>
  valueInCoins.trunc().div(10 ** currencyPrecision);
...
export default class Money {
  constructor(amount, currency) {
    ...
    if (amount instanceof BigNumber || amount._isBigNumber) amount = amount.toString();

    this.amount = new Decimal(amount).toDecimalPlaces(currency.precision, Decimal.ROUND_FLOOR);
  ...
  static fromCoins = (amount, currency) => {
    currency = currency || {};
    if (currency.precision === undefined) throw new Error('A valid currency must be provided');

    if (amount instanceof BigNumber || amount._isBigNumber) amount = amount.toString();

    amount = new Decimal(amount);
    amount = amount.div(10 ** currency.precision);
  ...
  multiply = (multiplier) => {
    ...
    if (Number.isNaN(multiplier)) throw new Error('Multiplication by NaN is not supported');
...
Decimal.config({ toExpNeg: -(Currency.DCC.precision + 1) });
