...
export const createListItem = (transaction, dApps) => {
  switch (transaction.type) {
    ...
    case 16:
      return (
        <ScriptInvocationTransactionListItem key={transaction.id} tx={transaction} dApps={dApps} />
      );
...
export class Line extends React.PureComponent {
  ...
  render() {
    let className = 'line';
    if (!this.props.wrap) className += ' no-wrap';
    if (this.props.bold) className += ' bold';
  ...
  render() {
    return (
      <td data-label="ID / Timestamp">
        <Line wrap={false}>
          {(this.props.applicationStatus === 'script_execution_failed' ||
            this.props.applicationStatus === 'elided') && <FailedBrick />}
          <TransactionRef txId={this.props.id} />
        </Line>

        <Line>
          <span>
            <Timestamp value={this.props.timestamp} />
          </span>
        </Line>
      </td>
    );
      ...
      <td data-label="Amount / Fee">
        <Line>
          {Array.isArray(amount)
            ? amount.map((v) => <p key={v.toString()}>{v.toString()}</p>)
            : amount.toString()}
        </Line>
        <Line>
          <span>{this.props.fee.toString()}</span>
        </Line>
      </td>
    );
  }
    ...
    return (
      <tr>
        ...
        <td data-label="Sender / Recipient">
          ...
          <Line wrap={false}>
            <EndpointRef endpoint={tx.recipient} appearance="regular" />
          </Line>
        </td>
        <AmountAndFee amount={tx.amount} fee={tx.fee} />
        <td data-label="Price">
          <Line>
            <CurrencyRef currency={tx.amount.currency} />
          </Line>
        </td>
      </tr>
    );
      ...
      <tr>
        ...
        <td data-label="Seller / Buyer">
          ...
          <div className="line no-wrap">
            <EndpointRef endpoint={tx.seller} appearance="regular" />
          </div>
          <div className="line no-wrap">
            <EndpointRef endpoint={tx.buyer} appearance="regular" />
          </div>
        </td>
        ...
        <td data-label="Pair / Price">
          <div className="line">
            <CurrencyRef currency={tx.price.amountAsset} /> /{' '}
            <CurrencyRef currency={tx.price.priceAsset} />
          </div>
    ...
    return (
      <tr>
        ...
        <td data-label="Sender / Recipient">
          ...
          <Line wrap={false}>{tx.assetId && <RoutedAssetRef assetId={tx.assetId} />}</Line>
        </td>
        <AmountAndFee amount={tx.amount} fee={tx.fee} />
        <td>
          <Line bold={true}>
  ...
  render() {
    ...
    return (
      <tr className={rowClassName}>
        ...
        <td data-label="Price">
          <Line>
            <CurrencyRef currency={tx.totalAmount.currency} />
          </Line>
        </td>
      </tr>
    );
  ...
  render() {
    ...
    return (
      <tr>
        ...
        <Subjects type={tx.type} sender={tx.sender} />
        <JustFee fee={tx.fee} />
        <td data-label="Price">{tx.sponsoredFee && <Line>{tx.sponsoredFee.toString()}</Line>}</td>
      </tr>
    );
  }
}
                ...
                {dApps[tx.dappAddress] ? (
                  <div>
                    <div className="badge dapp-link">
                      <EndpointRef
                        endpoint={tx.sender}
                        title={dApps[tx.dappAddress]}
                        appearance="regular"
                      />
                    </div>
                  </div>
                ) : (
                  <EndpointRef endpoint={tx.dappAddress} appearance="regular" />
                )}
              </div>
            )}
          </Line>
