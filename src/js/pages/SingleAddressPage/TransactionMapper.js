...
const mapScriptInvocation = (tx, currentAddress) => {
  ...
  const payment = tx.payment
    ? Array.isArray(tx.payment)
      ? tx.payment.map((v) => moneyToObject(v))
      : [moneyToObject(tx.payment)]
    : null;
...
const mapMassTransfer = (tx, currentUser) => {
  ...
  if (matchesUser(currentUser, tx.sender)) {
    ...
    tx.transfers
      .filter((transfer) => matchesUser(currentUser, transfer.recipient))
      .forEach((t) => {
...
const mapExchange = (tx, _currentAddress) => {
  return Object.assign(copyMandatoryAttributes(tx), {
    sender: tx.seller,
    recipient: tx.buyer,
