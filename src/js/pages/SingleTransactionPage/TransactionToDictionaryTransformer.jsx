...
const scriptInvocationTransactionToItems = (tx, _networkId, _dApps) => {
  const paymentItems = [
    {
      label: 'Payments',
      value:
        tx.payment && tx.payment.length > 0 ? (
          ...
          >
            {tx.payment.map((v) => (
              <MoneyInfo key={`${v.amount}_${v.currency}`} value={v} />
            ))}
          </div>
        ) : (
          ''
  ...
  const results =
    tx.stateUpdate || tx.stateChanges
      ? [
          {
            label: 'Results',
  ...
  if (tx.isEthereum)
    info.default.splice(-[...stateItems, ...results].length, 0, buildBytesItem(tx));
  return info;
};

const _invokeExpressionTransactionToItems = (tx) => {
  const stateItems = tx.stateChanges
    ? [
        {
          label: 'State Changes',
  ...
  const results =
    tx.stateUpdate && tx.applicationStatus === 'succeeded'
      ? [
          {
            label: 'Results',
            value: <StateUpdateInfo tx={tx} />,
...
const exchangeTransactionToItems = (tx) => {
  ...
  headerItems.splice(1, 0, {
    ...
    value: (
      <React.Fragment>
        <CurrencyRef currency={assetPair.amountAsset} /> /{' '}
        <CurrencyRef currency={assetPair.priceAsset} />
      </React.Fragment>
    ),
  });
...
const _ethereumTransactionToItems = (_tx) => {
  const _items = [];
};
...
const buildDescriptionItem = (tx) => {
  const value = <Description text={tx.description} />;
    ...
    },
    {
      ...
      value:
        tx.applicationStatus === 'script_execution_failed' ? (
          <>
            <img src={brick} alt="Script execution failed" height={12} width={12} />
            &nbsp;Script execution failed
          </>
        ) : tx.applicationStatus === 'elided' ? (
          <>
            <img src={brick} alt="Elided" height={12} width={12} />
            &nbsp;Elided
          </>
        ) : tx.applicationStatus === 'script_execution_in_progress' ? (
          <>
            <img src={pending} alt="In progress" height={12} width={12} />
...
const buildSenderAddressAndKeyItems = (tx) => [buildSenderItem(tx), buildSenderPublicKeyItem(tx)];

const buildQuantityItem = (tx) => {
  const value = tx.amount.toString();
