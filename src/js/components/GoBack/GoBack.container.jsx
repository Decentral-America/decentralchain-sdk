...
class GoBack extends React.PureComponent {
  ...
  componentDidMount() {
    if (window.history.length === 1) this.setState({ enabled: false });
  }
  ...
  render() {
    ...
    return (
      <button type="button" className="back" onClick={this.handleClick}>
        <span className="btn btn-back"></span>
        <span className={linkClassName}>Back</span>
      </button>
    );
  }
}
