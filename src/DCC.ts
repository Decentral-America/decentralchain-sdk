...
const DCC_CONFIG = {
    ...
    // NOTE: SECRET must remain 'WAVES' — this is the Ledger firmware app identifier
    // that must match the installed firmware. Do NOT change unless DCC has its own Ledger app.
    SECRET: 'WAVES',
    ...
    MAX_SIZE: 128,
    DCC_PRECISION: 8,
    MAIN_NET_CODE: 76, // 'L' for DCC mainnet
};
...
export class DCC {
    ...
    constructor(transport: any, networkCode = DCC_CONFIG.MAIN_NET_CODE) {
    ...
    async getWalletPublicKey(path: string, verify = false): Promise<IUserData> {
        ...
        const address = response
            .slice(DCC_CONFIG.PUBLIC_KEY_LENGTH, DCC_CONFIG.PUBLIC_KEY_LENGTH + DCC_CONFIG.ADDRESS_LENGTH)
            .toString('ascii');
        const statusCode = response
            .slice(-DCC_CONFIG.STATUS_LENGTH)
            .toString('hex');
    ...
    async signOrder(path: string, sOData: ISignOrderData): Promise<string> {
        ...
        sData.dataType = DCC_CONFIG.SIGNED_CODES.ORDER
        const dataForDevice = await this._fillDataForSign(path, sData);
        return await this._signData(dataForDevice);
    }
    ...
    async signSomeData(path: string, sOData: ISignData): Promise<string> {
        const sData = sOData as ISignTxData;
        sData.dataType = DCC_CONFIG.SIGNED_CODES.SOME_DATA
        sData.dataVersion = 0;
    ...
    async signRequest(path: string, sOData: ISignData): Promise<string> {
        const sData = sOData as ISignTxData;
        sData.dataType = DCC_CONFIG.SIGNED_CODES.REQUEST
        sData.dataVersion = 0;
        sData.amountPrecision = 0;
        sData.feePrecision = 0;
        const dataForDevice = await this._fillDataForSign(path, sData);
    ...
    async signMessage(path: string, sOData: ISignData): Promise<string> {
        const sData = sOData as ISignTxData;
        sData.dataType = DCC_CONFIG.SIGNED_CODES.MESSAGE
        sData.dataVersion = 0;
        sData.amountPrecision = 0;
        sData.feePrecision = 0;
        const dataForDevice = await this._fillDataForSign(path, sData);
    ...
    protected async _fillDataForSign(path: string, sData: ISignTxData) {
        ...
        if (appVersion[0] >= 1 && appVersion[1] >= 2 && appVersion[2] >= 0) {
            const prefixData = Buffer.concat([
                ...
                new Buffer(DCC._toInt32Bytes(sData.dataBuffer.byteLength))
            ]);
    ...
    static checkError(data: Array<number>): { error: string, status: number } | null {
        ...
        if (statusCode === DCC_CONFIG.STATUS.SW_OK) {
            return null;
        }
